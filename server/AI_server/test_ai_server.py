...
# 🟢 [수정된 헬퍼 함수] Max STD 기반 포즈 분석 (단일 발목 대체 로직 포함)
def analyze_pose_by_max_std(track_id, kpts, bbox, history_dict, conf_thresh, window_size, stop_thresh, run_thresh):
    status = "초기화 중..."
    debug_norm_max = 0.0
    ...
    std_values = [] # 유효한 정규화된 STD 값이 저장될 목록
    ...
    if l_y != -1: 
        ...
        if len(history_dict[track_id]['left_y']) == window_size:
            ...
            norm_std_l = std_l / norm_factor
            std_values.append(norm_std_l)
    else: 
    ...
    if r_y != -1: 
        ...
        if len(history_dict[track_id]['right_y']) == window_size:
            ...
            norm_std_r = std_r / norm_factor
            std_values.append(norm_std_r)
    else: 
        history_dict[track_id]['right_y'].clear()

    # 2. 분류 결정
    if std_values:
        # 🟢 [핵심 로직] 보이는 모든 발목 STD 중 최대값 사용 (단일 발목 대체)
        max_norm_std = max(std_values)
        debug_norm_max = max_norm_std
        
        if max_norm_std < stop_thresh:
            status = "정지 (Stoping)"
        elif max_norm_std < run_thresh:
            status = "걷는 중 (Walking)"
        else:
            status = "뛰는 중 (Running)"

    elif l_y == -1 and r_y == -1: 
        status = "측정 불가 (Ankles 안보임)"
        
    return status, debug_norm_max
...
class YoloActionServer(Node):
    def __init__(self):
        ...
        # --- 2. 설정값 (Max STD 규칙 및 BBox Growth) ---
        self.SMOOTHING_WINDOW = 15 
        self.KEYPOINT_CONF_THRESHOLD = 0.5
        
        # 🟢 [Max STD 임계값]
        self.MAX_STD_STOP_THRESHOLD = 0.008      
        self.MAX_STD_WALK_THRESHOLD = 0.040
        ...
        self.get_logger().info(f"🔥 Max STD 규칙: Stop < {self.MAX_STD_STOP_THRESHOLD}, Run > {self.MAX_STD_WALK_THRESHOLD}, Growth > {self.BBOX_AREA_GROWTH_THRESHOLD}x")
            ...
            try:
                ...
                if results[0].boxes.id is not None and results[0].keypoints is not None:
                    ...
                    for box_raw, track_id, kpts_raw in zip(boxes, track_ids, keypoints_data):
                        ...
                        # 🟢 Max STD 기반 행동 분석 (단일 발목 대체 로직 사용)
                        status, debug_norm_max = analyze_pose_by_max_std(
                            track_id, kpts_raw.data.cpu().numpy(), box_np, self.status_history, 
                            self.KEYPOINT_CONF_THRESHOLD, self.SMOOTHING_WINDOW,
                            self.MAX_STD_STOP_THRESHOLD, self.MAX_STD_WALK_THRESHOLD 
                        )
                        
                        # 📝 화면 표시 내용 구성
                        line1 = f"ID: {track_id} | 상태: {status}"
                        line2 = f"  └ Max_STD: {debug_norm_max:.4f} (R Th:{self.MAX_STD_WALK_THRESHOLD})"
                ...
                cv2.imshow("YOLO & ArUco Fusion Server (Max STD Priority)", annotated_frame)
